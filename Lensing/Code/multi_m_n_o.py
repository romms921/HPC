...
# --- File Paths ---
model_output_dir = '/home/rommulus/Projects/itng_lensing/Simulations/Output' # Model Save Path
base_results_path = '/home/rommulus/Projects/itng_lensing/Simulations/Input/System_2' # Path with all input files
log_file_path = '/home/rommulus/Projects/itng_lensing/Simulations/Run/simulation_log.txt' # Log file path
restart_file_path = os.path.join(os.path.dirname(log_file_path), 'simulation_restart_state_var.json') # Restart file
obs_point_file = base_results_path + '/pos_point.dat'  # Observation file path ( Should include all available data )
crit_file = model_output_dir + '/critical_curves.csv' # Critical curve output file path


# --- Simulation Parameters ---
sim_name = 'Sim 8' # Name of Sim
model = 'POW' # Model Type
...
prior_file = base_results_path + '/prior.dat'  # Path to prior file
critical_curve = False # Whether to save critical curves and caustics (Very large file size )
time_delay = False # Whether to extract time delay information
h0 = False # Whether to extract H0 value
input_py_file = base_results_path + '/pow_input.py' # Input Python file path

# --- Performance & Resource Management ---
NUM_PROCESSORS = int(os.getenv("SLURM_NTASKS", multiprocessing.cpu_count()))
CHUNK_SIZE = 100000  # Sims split into chuncks of this size
CONSOLE_UPDATE_INTERVAL = 100000 # Update console output every N iterations
...
def rms_extract(model_ver, model_path, temp_input_py_file):
    global pos_rms, mag_rms, chi2_value, source_params, hubble, td
    ...
    return_values = (pos_rms, mag_rms, dfs, chi2_value, source_params)
    if time_delay:
        return_values += (td,)
    if h0:
        return_values += (hubble,)
...
# Critical curve and caustic file creation
if critical_curve:
    crit_dir = os.path.dirname(crit_file) or '.'
    os.makedirs(crit_dir, exist_ok=True)
    if not os.path.exists(crit_file):
        try:
            crit_dat = pd.DataFrame(columns=['x_crit_1', 'y_crit_1', 'x_caust_1', 'y_caust_1'])
            crit_dat.to_csv(crit_file, index=False)
        except Exception as e:
            print(f"Warning: could not create critical curve file '{crit_file}': {e}")
...
def run_single_model(params):
    ...
    try:
        ...
        for line_num, line in enumerate(lines):
            ...
            elif line.strip().startswith('critical_curve ='):
                lines[line_num] = f"critical_curve = {critical_curve}\n"

        for line_num, line in enumerate(lines):
            if prior_file and line.strip().startswith('prior_file ='):
                lines[line_num] = f"prior_file = '{prior_file}'\n"
        ...
        subprocess_env.update({"MKL_NUM_THREADS":"1","NUMEXPR_NUM_THREADS":"1","OMP_NUM_THREADS":"1","OPENBLAS_NUM_THREADS":"1","VECLIB_MAXIMUM_THREADS":"1"})
        subprocess.run(['python3', temp_input_py_file], env=subprocess_env, check=True, capture_output=True)

        results = rms_extract(model_name, model_output_dir, temp_input_py_file)

        # Unpack the results dynamically
        pos_rms, mag_rms, dfs, chi2, source = results[:5]
        result_idx = 5
        if time_delay:
            td = results[result_idx]
            result_idx += 1
        if h0:
            hubble = results[result_idx]
        ...
        if time_delay:
            result_dict['time_delay'] = td
        else:
            pass

        if h0:
            result_dict['hubble'] = hubble
        else:
            pass

        # Handling of Critical Curves and Caustics
        if critical_curve:
            crit_curve_file = os.path.join(model_output_dir, f'{model_name}_crit.dat')
            crit_data = pd.read_csv(crit_curve_file, sep='\s+', header=None, names=['x_crit_1', 'y_crit_1', 'x_caust_1', 'y_caust_1', 'x_crit_2', 'y_crit_2', 'x_caust_2', 'y_caust_2'])
            masked_crit_data = crit_data.iloc[:, :4]  # keep only the first four columns
            masked_crit_data.to_csv(crit_file, mode='a', index=False, header=False)
        ...
        if critical_curve:
            crit_curve_file = os.path.join(model_output_dir, f'{model_name}_crit.dat')
            if os.path.exists(crit_curve_file):
                os.remove(crit_curve_file)
